...
| `spawn_waypoint_road_id`          | int    | -1                                                                                | OpenDRIVE road id for waypoint-based spawn. When >= 0, the ego vehicle is spawned at the waypoint given by (`spawn_waypoint_road_id`, `spawn_waypoint_lane_id`, `spawn_waypoint_s`), taking precedence over `spawn_point` |
| `spawn_waypoint_lane_id`          | int    | 0                                                                                 | OpenDRIVE lane id for waypoint-based spawn                                                                                                                                                                          |
| `spawn_waypoint_s`                | double | 0.0                                                                               | OpenDRIVE s coordinate [m] for waypoint-based spawn                                                                                                                                                                 |
| `use_pov_highway`                 | bool   | False                                                                             | Preset that loads the `PoV_Highway` map and spawns the `vehicle.carlamotors.european_hgv` truck at the `pov_highway_demo.py` location (road_id=2, lane_id=-2, s=500). Also selects `carla_hgv_sensor_kit_description` as the sensor kit; launch Autoware with `sensor_model:=carla_hgv_sensor_kit` so the TF tree matches |
...
The interface uses the **`carla_sensor_kit`** which provides 6 cameras for 360-degree coverage, LiDAR, IMU, and GNSS sensors. A second kit, **`carla_hgv_sensor_kit`**, has the same sensors but places them on the European HGV truck (cameras on the cab front, LiDAR on the cab roof); it is selected automatically with `use_pov_highway:=true`. Sensor configurations are dynamically loaded from Autoware sensor kit calibration files through two configuration files:
